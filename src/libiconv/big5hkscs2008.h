...
static int
big5hkscs2008_mbtowc (ucs4_t *pwc, const unsigned char *s, size_t n)
{
  ...
  if (last_wc) {
    ...
    *pwc = last_wc;
    ...
    {
      int ret = hkscs1999_mbtowc(pwc,s,n);
      if (ret != RET_ILSEQ)
        return ret;
    }
    {
      int ret = hkscs2001_mbtowc(pwc,s,n);
      if (ret != RET_ILSEQ)
        return ret;
    }
    {
      int ret = hkscs2004_mbtowc(pwc,s,n);
      if (ret != RET_ILSEQ)
        return ret;
    }
    {
      int ret = hkscs2008_mbtowc(pwc,s,n);
      if (ret != RET_ILSEQ)
        return ret;
    }
    if (c == 0x88) {
...
static int
big5hkscs2008_wctomb (unsigned char *r, ucs4_t wc, size_t n)
{
  int count = 0;
...
static int
big5hkscs2008_reset (unsigned char *r, size_t n)
{
  unsigned char last = conv->ostate;
