...
static int
gb18030_wctomb (unsigned char *r, ucs4_t wc, size_t n)
{
  int ret;
  ...
  /* Code set 1 (GBK extended) */
  ret = gbk_wctomb(r,wc,n);
  if (ret != RET_ILUNI)
    return ret;

  ret = gb18030ext_wctomb(r,wc,n);
  if (ret != RET_ILUNI)
    return ret;
      ...
      if (wc < 0xe766) {
        ...
        while (k1 < k2) {
          ...
          if (wc < gb18030_pua2charset[k*3+0])
            ...
            unsigned short c =
              static_cast<unsigned short>(gb18030_pua2charset[k*3+2] + (wc - gb18030_pua2charset[k*3+0])); // narrow_cast yea
            r[0] = (c >> 8);
            r[1] = (c & 0xff);
            return 2;
          }
