...
static int
iso8859_15_mbtowc (ucs4_t *pwc, const unsigned char *s, size_t n)
{
  unsigned char c = *s;
  if (c >= 0xa0 && c < 0xc0)
...
static int
iso8859_15_wctomb (unsigned char *r, ucs4_t wc, size_t n)
{
  unsigned char c = 0;
  if (wc < 0x00a0) {
    ...
    c = static_cast<unsigned char>(wc); // narrow_cast
